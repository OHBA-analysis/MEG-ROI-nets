...
if strcmpi(Settings.paradigm, 'rest'),
    ...
    [T, p, corrp] = ROInets.univariate_edge_test(correlationMats{iFreq}.envCorrelation_z, ...
    ...
    [T, p, corrp] = ROInets.univariate_edge_test(correlationMats{iFreq}.envPartialCorrelation_z, ...
                                                 Settings.GroupLevel.designMatrix,        ...
                                                 Settings.GroupLevel.contrasts);
    for iContrast = size(p,3):-1:1,
        h(:,:,iContrast) = ROInets.false_discovery_rate(ROInets.p_to_z_two_tailed(p(:,:,iContrast)), ...
    ...
    [T, p, corrp] = ROInets.univariate_edge_test(correlationMats{iFreq}.envPartialCorrelationRegularized_z, ...
                                                 Settings.GroupLevel.designMatrix,        ...
                                                 Settings.GroupLevel.contrasts);
    for iContrast = size(p,3):-1:1,
        h(:,:,iContrast) = ROInets.false_discovery_rate(ROInets.p_to_z_two_tailed(p(:,:,iContrast)), ...
...
elseif strcmpi(Settings.paradigm, 'task'),
    ...
    for iContrast = length(Settings.SubjectLevel.contrasts):-1:1,
        ...
        [T, p, corrp] = ROInets.univariate_edge_test(correlationMats{iFreq}.firstLevel(iContrast).cope.partialCorrelation, ...
                                                     Settings.GroupLevel.designMatrix,                                     ...
                                                     Settings.GroupLevel.contrasts);
        for iConG = size(p,3):-1:1,
            h(:,:,iConG) = ROInets.false_discovery_rate(ROInets.p_to_z_two_tailed(p(:,:,iConG)), ...
        ...
        if Settings.Regularize.do,
            [T, p, corrp] = ROInets.univariate_edge_test(correlationMats{iFreq}.firstLevel(iContrast).cope.partialCorrelationRegularized, ...
                                                         Settings.GroupLevel.designMatrix,                                                ...
                                                         Settings.GroupLevel.contrasts);
            for iConG = size(p,3):-1:1,
                h(:,:,iConG) = ROInets.false_discovery_rate(ROInets.p_to_z_two_tailed(p(:,:,iConG)), ...
